...
export const CHATGPT_STARRED_MESSAGES_STORAGE_KEY = 'chatgptVoyager.starredMessages';
...
export async function listChatGPTStarredMessages(): Promise<ChatGPTStarredMessage[]> {
  ...
  const raw = result[CHATGPT_STARRED_MESSAGES_STORAGE_KEY];
  if (!Array.isArray(raw)) return [];

  return sortStarredMessages(
