...
- Timeline 增强：
  ...
  - hover 预览；
  - 匹配关键词高亮；
  - 性能优化：打开 ChatGPT 对话时优先展示原生对话内容，搜索和时间轴所需的完整 conversation 数据延迟加载；
  - 搜索和时间轴数据加载中时，右侧时间轴面板显示“加载中”，加载完成后再显示节点和搜索结果；
  - 避免因时间轴 / 搜索预处理阻塞或显著拖慢 ChatGPT 对话正文展示。
- Prompt Variables 增强：
  - 支持 prompt 变量；
  - 使用前弹出变量填写表单；
...
- hover 预览可用。
- 关键词高亮可用。
- 打开 ChatGPT 对话时，不因时间轴或搜索索引构建显著延迟正文展示。
- 时间轴 / 搜索数据未就绪时有“加载中”状态，数据就绪后自动恢复正常显示。
- Prompt 变量填写和插入可用。
- Chrome 构建成功。
